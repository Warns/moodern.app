...
This policy describes how the **Moodern** iOS app (“the App”) handles information on your device.

---

## Summary

Moodern is designed so your **mood logs and related notes stay on your iPhone**. We do **not** operate our own servers to receive, store, or analyze your mood data. We do **not** use third-party analytics, advertising SDKs, or crash-reporting services that send your mood content to us in the current version of the App.
...
This data remains in the App’s **local database** on your device unless you take an action that copies it elsewhere (see **Export** below).
...
- Operate backend servers that sync or store your mood entries
- Sell your mood data
- Use in-App third-party analytics or advertising tools that transmit your mood entries to the Developer

If this ever changes in a future version, we will update this policy and, where required, the App Store disclosures **before** expanding collection.

---

## iCloud and device backup

Apple may include App data in **iCloud Backup** or **local device backups** if you have those features enabled. Those backups are managed by **Apple** and your Apple ID, not by the Developer’s servers. Moodern does not add separate “cloud sync” of your mood database to the Developer’s infrastructure in the current version.

---

## Children

The App is not directed at children in a way that changes this local-only design. Parents and guardians should review device and Apple ID settings (including Family Sharing and Screen Time) if a child uses the device.

---

## Security

Your mood data is protected by **standard iOS protections** for app sandboxing and device security (passcode, Face ID, Touch ID, etc.). You are responsible for keeping your device and Apple ID secure.
